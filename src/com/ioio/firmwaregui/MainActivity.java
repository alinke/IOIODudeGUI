...
public class MainActivity  {
    ...
    private static String ourNodeName = "/com/ioio/firmwaregui";
	...
	portLabel_ = new JLabel("IOIO Port", JLabel.LEFT);
	...
	combinedText = "FIRMWARE UPGRADE INSTRUCTIONS\n"
		    + "STEP 1\n"
			+ "- Power off the IOIO (detach it from a power supply AND from the PC,\n" 
			+ "until the red pwr LED is off).\n"
			+ "- Jumper the boot pin (near pin number 34) to GND.\n"
			+ "- With the jumper in place, plug the IOIO to your PC, using a micro-B cable. The IOIO should\n" +
			" power on (red pwr LED is on) and the yellow stat LED should be on as well (in fact, the boot pin\n " +
			"in connected in parallel to the IOIO pin which controls the LED, so by jumpering boot and\n " +
			"GND you are forcing it to light!).\n"
			+ "- Now, remove the jumper. The stat LED should blink a few times, telling you that\n " +
			"the bootloader is now running.\n"
			+ "- Once done blinking, the IOIO should enumerate itself on your PC (i.e. you should see it in\n " +
			"the Devices screen in Windows or in lsusb on Linux).\n"
			+ "STEP 2\n"
	...
	versionButton.addActionListener(new ActionListener(){
        ...
        public void actionPerformed(ActionEvent e){
		    ...
		    if (portText.getText().equals("Enter IOIO Port Here")) {
		    	...
		    	combinedText = "IOIO PORT FORMAT IS NOT VALID\n\n"
		    			+  serialPortInstructions + "\n";
    		    mainText.setText(combinedText);
		    }
	...
	upgradeButton.addActionListener(new ActionListener(){
        ...
        public void actionPerformed(ActionEvent e){
		    ...
		    else if (portText.getText().equals("Enter IOIO Port Here")) {
		    	combinedText = "Oops... You forgot to enter IOIO's Port\n\n"
		    			+ serialPortInstructions + "\n";
    		    mainText.setText(combinedText);
		    }
	...
	public static void createAndShowGUI() {
		...
		String defaultPortValue = "Enter IOIO Port Here"; //if the pref does not exist yet, use this
		portName_ = prefs.get(prefPort_, defaultPortValue);
	 		
		frame = new JFrame("IOIO Firmware Upgrade");
	...
	private void runFirmwareUpdate() {
		combinedText = "IOIO FOUND" + "\n"
				+ "Current IOIO Firmware Version: " + pixelFirmware;
		mainText.setText(combinedText);
	}
	...
	private static void writeCommand() throws IOException, ProtocolException,
		...
		try {
			...
			if (entry == null) {
				System.err
						.println("Application bundle does not include an image for the platform "
								+ platformVersion_);
				combinedText = combinedText + "\n" + "**** THIS  FIRMWARE FILE IS NOT VALID ****\n\n";
			    mainText.setText(combinedText);
				return;
			}
			...
			if (readChecksum() != checksum) {
				combinedText = combinedText + "Bad checksum.  firmware image is possibly corrupt.";
			    mainText.setText(combinedText);
	...
	private static void checkBootloaderProtocol() throws IOException,
		...
		if (whatIsConnected_ != Protocol.PROTOCOL_BOOTLOADER) {
			...
			combinedText = combinedText + "**** IOIO WAS DETECTED BUT IS NOT IN BOOTLOADER MODE ****\n\n"
	...
	private static void versionsCommand() {
		...
		case PROTOCOL_IOIO:
			System.err.println("IOIO FOUND\n\n");
			combinedText =  "IOIO FOUND\n";
		    mainText.setText(combinedText);
			break;

		case PROTOCOL_BOOTLOADER:
			System.err.println("IOIO is in bootloader mode");
			combinedText = "IOIO is in bootloader mode. Please power IOIO on and off if you're down with the firmware upgrade and ready to use .\n\n";
		    mainText.setText(combinedText);
			break;
		}
		System.err.println();
	...
	private static void connect(String port) throws IOException,
		...
		if (bufferIsIOIO()) {
			...
			combinedText = combinedText + "Device is neither a standard  application nor a  bootloader." + "\n";
		    mainText.setText(combinedText);
			throw new ProtocolException(
					"Device is neither a standard  application nor a  bootloader.");
		}
}
